...
const RecentTransactions = () => {
  ...
  console.log("filtered ___________________>>>>>> " , filtered);
    ...
    const fetchBoth = async () => {
      ...
      try {
        ...
        const [depRes, wdRes] = await Promise.allSettled([
          backendApi.get(`/client/deposit/${loggedUser._id}`, { signal: controller.signal as any }),
          backendApi.get(`/client/withdrawals/${loggedUser._id}`, { signal: controller.signal as any }),
        ...
        const wdPayload = wdRes.status === "fulfilled" ? wdRes.value?.data?.data : [];
             ...
             console.log("depPayload -------------->>>>>>>>>>>>> " , depPayload);
        console.log("wdPayload -------------->>>>>>>>>>>>> " , wdPayload);
        ...
        console.log("merged ------------------>>>>>0" , merged) ;
        

        setTransactions(merged);
        // if (merged.length === 0) setError("No transactions found.");
      } catch (err: any) {

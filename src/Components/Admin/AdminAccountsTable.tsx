...
const Chip = ({ children, tone = "blue" }: { children: ReactNode; tone?: Tone }) => {
...
export default function UserAccountsTable({ title = "Accounts", subtitle = "User trade accounts", className, onView }: Props) {
  const { id } = useParams();
  ...
  const fetchUserData = async () => {
    ...
    setLoading(true);
    try {
      const res = await backendApi.get(`/admin/get-user?id=${id}`);
  ...
  return (
    <div className={cx("transition-all duration-500", mounted ? "opacity-100 translate-y-0" : "opacity-0 translate-y-2", className)}>
      <CardShell className="p-0 overflow-hidden">
        {/* header */}
        <div className="px-5 sm:px-6 py-4 border-b" style={{ borderColor: THEME.borderSoft, background: "rgba(0,0,0,0.25)" }}>
            ...
            <tbody>
              {loading ? (
                ...
                accounts.map((acc: any, idx: number) => {
                  ...
                  const timestamp =
                    acc?.createdAt || acc?.timestamp ? CFformatDate(acc?.createdAt || acc?.timestamp) : "—";

                  return (
                    <tr
                      key={acc?._id || `${accountNumber}-${idx}`}
                      className={cx("transition-all duration-300 hover:scale-[1.005]")}
                      style={{
                    ...
                    >
                      <TableCell className="font-black whitespace-nowrap">
                        <span className="inline-flex items-center gap-2">
                          <span className="h-2 w-2 rounded-full" style={{ background: THEME.blue, boxShadow: `0 0 12px ${THEME.blue}88` }} />
                      ...
                      <TableCell className="text-right whitespace-nowrap">
                        <div className="flex justify-end gap-2">
                          <ActionPill label="Acc" tone="blue" icon={<Copy className="w-4 h-4" />} onClick={() => safeCopy(accountNumber, "Account copied")} />
                          <ActionPill label="Master" tone="orange" icon={<KeyRound className="w-4 h-4" />} onClick={() => safeCopy(masterPassword, "Master password copied")} />
                          <ActionPill label="Investor" tone="teal" icon={<KeyRound className="w-4 h-4" />} onClick={() => safeCopy(investorPassword, "Investor password copied")} />
                          <ActionPill label="View" tone="purple" icon={<Eye className="w-4 h-4" />} onClick={() => onView?.(acc)} />
                        </div>
                      </TableCell>
                    </tr>
                  );

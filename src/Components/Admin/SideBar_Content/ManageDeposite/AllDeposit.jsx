...
const AllDeposit = () => {
  ...
  const fetchDeposits = async () => {
    try {
      ...
      const res = await axios.get(
        `${API_URL}/api/deposits?page=${page}&limit=10&search=${search}&status=`,
        config
      );
  ...
  return (
    <div className="bg-gradient-to-br from-indigo-900 via-indigo-950 to-indigo-900 rounded-xl p-6 shadow-lg text-white">
      <h2 className="text-lg font-semibold mb-4">All Deposits</h2>
      ...
      {loading ? (
        ...
        <p className="text-gray-300">No deposits found.</p>
      ) : (
            ...
            <tbody>
              {deposits.map((d) => (
                ...
                >
                  ...
                  <td className="p-3 border border-gray-700 text-blue-300">
                    {d.userId?.email}
                  </td>
